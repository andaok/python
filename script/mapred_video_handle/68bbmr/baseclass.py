...
class interWithMysql():
    ...
    def writeStatus(self,flag,iswmark,isrewrite,info=None):
        ...
        try:
            ...
            dbconn.select_db('weitongnian')
            
            if int(status) == 22:
                if iswmark:
                    #有打水印
                    dbcursor.execute('update t_videoinfo set status=%s, water=%s where vid=%s',(status,1,self.vid))
                    self.logger.info("Congratulation,t_videoinfo and watermark handle success!!!,info : %s"%info)
                else:
                    #没打水印
                    dbcursor.execute('update t_videoinfo set status=%s where vid=%s',(status,self.vid))
                    self.logger.info("update t_videoinfo set status=%s where vid=%s"%(status,self.vid))
                    self.logger.info("Congratulation,t_videoinfo handle success!!!,info : %s"%info)
            
            elif int(status) == 23:
                #此种情况肯定是打了水印的
                dbcursor.execute('update t_videoinfo set status=%s, water=%s where vid=%s',(status,1,self.vid))
                self.logger.info("Congratulation,t_videoinfo and rewatermark handle success!!!,info : %s"%info)
                
            elif int(status) == 12:
                dbcursor.execute('update t_videoinfo set status=%s where vid=%s',(status,self.vid))
                if iswmark:
                    #有打水印
                    self.logger.info("Sorry,t_videoinfo and watermark handle fail!!!,Error info:%s"%info)
                else:
                    #没打水印
                    self.logger.info("Sorry,t_videoinfo handle fail!!!,Error info:%s"%info)
            elif int(status) == 34:
                #此种情况肯定是打了水印的
                dbcursor.execute('update t_videoinfo set status=%s where vid=%s',(status,self.vid))
                self.logger.info("Sorry,t_videoinfo and rewatermark handle fail!!!,Error info:%s"%info)
                
            dbconn.commit()                          
        except Exception,e:
            self.logger.error("update t_videoinfo status fail !!!!,error info : %s"%e)
            sys.exit()   
    ...
    def writeVinfo(self,bitrate,vbitrate,abitrate,width,height,duration,size,format):
         ''' Write t_videoinfo base information to mysql ''' 
         ...
         try:
            ...
            dbcursor.execute('update t_videoinfo set size=%s,extension=%s,width=%s,height=%s,vcode_rate=%s,vcode_vrate=%s,vcode_arate=%s,duration=%s where vid=%s',params) 
            dbconn.commit()
         except Exception,e:
            self.logger.error("Write t_videoinfo base information to mysql failure,error info : %s"%e)
            sys.exit()        
    ...
    def getStatus(self):
        try:
            ...
            dbcursor = dbconn.cursor()
            dbconn.select_db('weitongnian')
            sql = "select status from t_videoinfo where vid=%s"
    ...
    def getPicStatus(self):
        try:
            ...
            dbcursor = dbconn.cursor()
            dbconn.select_db('weitongnian')
            sql = "select preview from t_videoinfo where vid=%s"
            sql_result = dbcursor.execute(sql,(self.vid)) 
            status = dbcursor.fetchone()
            dbconn.commit()
        except Exception,e:
...
def getlog(VideoFileNameAlias,logfile="/tmp/68bbmr.log",loglevel="debug"):
