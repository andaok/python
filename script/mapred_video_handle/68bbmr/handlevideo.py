...
cmd = "(time %s jar %s  -file /tmp/%s/mrdata/  -mapper mapper.py  -reducer reducer.py -input hdfs://%s/%s/inputfiles -output hdfs://%s/%s/%s -numReduceTasks 1) >> /tmp/68bbmr.log 2>&1"
