...
class mapCmdOutput():
    ...
    def setKeyFrames(self,nameflag,newbitrate):
        setKFCmd = "ffmpeg.1.0.1 -y -threads 0 -i /tmp/%s/avfiles/%s -acodec copy -vcodec h264 -pix_fmt yuv420p -profile:v baseline -force_key_frames %s -b:v %sk -s %sx%s -strict -2  /tmp/%s/avfiles/%s.%s"
    ...
    def addWm(self,wmwidth,nameflag,newbitrate):
        ## addWmCmd = "ffmpeg.1.0.1 -y -threads 0 -i /tmp/%s/avfiles/%s -acodec copy -vcodec h264 -pix_fmt yuv420p -profile:v baseline -force_key_frames %s -b:v %sk -vf \"movie=/tmp/%s/%s, scale=%s:-1 [wm];[in][wm] overlay=%s:%s [out]\" /tmp/%s/avfiles/%s.%s"
        addWmCmd = "ffmpeg.1.0.1 -y -threads 0 -i /tmp/%s/avfiles/%s -acodec copy -vcodec h264 -pix_fmt yuv420p -profile:v baseline -force_key_frames %s -b:v %sk -vf \"movie=/tmp/%s/%s, scale=-1:%s [wm];[in][wm] overlay=%s:%s [out]\" /tmp/%s/avfiles/%s.%s"
        for segname in self.SegVideoNameList:
    ...
    def addWmTcode(self,newbitrate,newwidth,height,wmwidth,nameflag):
        ## addWmTcodeCmd = "ffmpeg.1.0.1 -y -threads 0 -i /tmp/%s/avfiles/%s -vcodec h264 -pix_fmt yuv420p -profile:v baseline -force_key_frames %s -b:v %sk -s %sx%s -acodec copy -vf \"movie=/tmp/%s/%s, scale=%s:-1 [wm];[in][wm] overlay=%s:%s [out]\" /tmp/%s/avfiles/%s.%s"
        addWmTcodeCmd = "ffmpeg.1.0.1 -y -threads 0 -i /tmp/%s/avfiles/%s -vcodec h264 -pix_fmt yuv420p -profile:v baseline -force_key_frames %s -b:v %sk -s %sx%s -acodec copy -vf \"movie=/tmp/%s/%s, scale=-1:%s [wm];[in][wm] overlay=%s:%s [out]\" /tmp/%s/avfiles/%s.%s"
    ...
    def tranCode(self,newbitrate,newwidth,height,nameflag):
        tranCodeCmd = "ffmpeg.1.0.1 -y -threads 0 -i /tmp/%s/avfiles/%s -vcodec h264 -pix_fmt yuv420p -profile:v baseline -force_key_frames %s -b:v %sk -s %sx%s -acodec copy /tmp/%s/avfiles/%s.%s"
        newwidth = self.getNewFrameWidth(height)
        for segname in self.SegVideoNameList:
