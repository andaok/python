...
function calBestDr()
 {
	...
	while [ "$tdr" -le "$etime" ]; do
	      ...
	      echo "dr is : $tdr , tsum is : $tsum" >> /tmp/videohandle.log
...
function BaseSplitVideoToBlock()
  {
    ...
    echo "PART$i RAW Duration: Video $pdrv Audio $pdra" >> /tmp/videohandle.log
    echo "PART$i RAW Start:    Video $sdrv Audio $sdra" >> /tmp/videohandle.log
...
function SplitVideoToBlock()
 {
    ...
    do 
       ...
       echo "Call BaseSplitVideoToBlock $m $ss $t " >> /tmp/videohandle.log
...
if [ "$SegDuration" == "00:00:00.00" ] || [ "$SegDuration" == "" ]; then
   IsHaveStream="NO"
else
   IsHaveStream="YES"
fi
