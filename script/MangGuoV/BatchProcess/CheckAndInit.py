...
try:
    ...
    if isExists(logger,redpipe):
        ...
        # --------------------------------------/
        #没有任何播放数据,仍需调用php处理套餐流量接口.
        p = subprocess.Popen("/usr/bin/curl http://www.skygrande.com/tasks/pack",shell=True,stdout=subprocess.PIPE,stderr=subprocess.STDOUT)
        p.wait()
        if str(p.returncode) != "0":
           logger.error("call php interface fail,error info : %s"%(p.stdout.read()))
        else:
           logger.info("call php interface success,info : %s"%(p.stdout.read()))
        # --------------------------------------/
