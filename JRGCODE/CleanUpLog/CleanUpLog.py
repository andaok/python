...
def GetFileList(LogPath):
    """
    Traverse the log directory and get a list of the names 
    that will need to be packed
    """
    
    LogDir = os.path.dirname(LogPath)
    LogNameExp = os.path.basename(LogPath)

    #Default compression Pack yesterday's log data
    LogDate = (datetime.datetime.now()-datetime.timedelta(days=1)).strftime("%Y%m%d")
    ...
    try:
        ...
        for LogPath in LogPathList:
            if os.path.isfile(LogPath):
                if re.match(r'^/.*\.log$',LogPath) != None:
                    LogDir = os.path.dirname(LogPath)
                    LogName = os.path.basename(LogPath)
